...
if($_SERVER["REQUEST_METHOD"]=="POST"){
    ...
    //   }
      ...
      $query= query("insert into test (lid, cid, name, plantingArea, harvestArea, output, yield, unit, township,  year) values 
      ( ?, ?, ?, ?, ?, ?, ?, ?, ? ,?)", 
     [$admin, 1, $name, $plant, $harvest, $output, $yield, $unit, $township, $year]);
      //print_r($query);
      if($query){
        go("http://localhost:100/cropAdmin/english/test_insert.php?name=$name&&create=success");
